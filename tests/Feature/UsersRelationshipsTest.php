...
()
    {
        ...
        'id' => 5,
        'user_id' => $user->id,])->assertDatabaseMissing('comments', [
        'id' => 3,
        'user_id' => $user->id,
...
()
    {
        ...
        'accept' => 'application/vnd.api+json',
        'content-type' => 'application/vnd.api+json',])->assertStatus(204);
        $this->assertDatabaseHas('comments', [
        'id' => 1,
        'user_id' => null,
        ])->assertDatabaseHas('comments', [
...
*/
public function
it_returns_a_404_not_found_when_trying_to_add_relationship_to_a_non_existing_comment
()
    {
        $user = factory(User::class)->create();
        Passport::actingAs($user);
...
()
    {
        $user = factory(User::class)->create();Passport::actingAs($user);
        $comments = factory(Comment::class, 3)->make();
        $user->comments()->saveMany($comments);
        $this->patchJson("/api/v1/users/{$user->id}/relationships/
        comments",[
...
()
    {
        $user = factory(User::class)->create();
        Passport::actingAs($user);
        $comments = factory(Comment::class, 3)->make();$user->comments()->saveMany($comments);
        $this->patchJson("/api/v1/users/{$user->id}/relationships/
        comments",[
        'data' => [
        [
...
*/
public function
it_validates_that_the_type_member_is_given_when_updating_a_relationship
()
    {
        $user = factory(User::class)->create();
        Passport::actingAs($user);
        $comments = factory(Comment::class, 3)->make();
        $user->comments()->saveMany($comments);$this->patchJson("/api/v1/users/{$user->id}/relationships/
        comments",[
        'data' => [
        [
        'id' => '1',
...
*/
public function
it_validates_that_the_type_member_has_a_value_of_authors_when_updating_a_relationship
()
    {
        $user = factory(User::class)->create();
        Passport::actingAs($user);
...
*/
public function
it_does_not_include_related_resource_objects_when_an_include_query_param_is_not_given
()
    {
        $user = factory(User::class)->create();
        Passport::actingAs($user);
